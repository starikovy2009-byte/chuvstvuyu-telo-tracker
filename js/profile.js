...
export function renderProfile({ state, profile, ui, actions }) {
  ...
  const avatarWrap = el("div", { className: "profile-avatar-wrap" }, [profileAvatar,
    el("button", { className: "icon-button", type: "button", text: "✎", attrs: { "aria-label": "Изменить фотографию" }, on: { click: () => actions.toggleEdit(true) } })
  ]);
  ...
  PRACTICES.forEach(([key, icon, title]) => {
    ...
    rhythm.append(el("div", { className: "practice-row" }, [
      el("span", { className: "practice-icon", text: icon, attrs: { "aria-hidden": "true" } }),
      el("div", {}, [
  ...
  if (ui.profileEditing) {
    ...
    const form = el("form", { className: "profile-edit", on: { submit: (event) => {
